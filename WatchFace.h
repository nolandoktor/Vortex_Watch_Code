...
void CascadeFace::draw(DoubleBuffer *lBuffer)
{
  lBuffer->clear();
  if (h_dest != 0)
    lBuffer->setColorVal(h_idx, DoubleBuffer::R /*LedBuffer::RED*/, COLOR_RES/5);
  if (m_dest != 0)
  lBuffer->setColorVal(m_idx, DoubleBuffer::G /*LedBuffer::GREEN*/, COLOR_RES/5);
  if (s_dest != 0)
    lBuffer->setColorVal(s_idx, DoubleBuffer::B /*LedBuffer::BLUE*/, COLOR_RES/5);
  ...
  for (int i=h_; i >= 0; i--)
  {
    lBuffer->setColorVal(i, LedBuffer::RED, COLOR_RES);
  }
  for (int i=m_; i >= 0; i--)
  {
    lBuffer->setColorVal(i, LedBuffer::GREEN, COLOR_RES);
  }
  for (int i=s_; i >= 0; i--)
  {
...
/*
  ...
  if (hour_ < 12)
  { 
    lBuffer->setColorVal(hour_, LedBuffer::RED, COLOR_RES);
  }

  lBuffer->setColorVal(min_idx, LedBuffer::GREEN, min_val);
  lBuffer->setColorVal(sec_idx, LedBuffer::BLUE, sec_val);
*/

  lBuffer->clear();
/*  
  int min_start = min_idx;
  int min_end = hour_;
  int hour_start = hour_;
  int hour_end = min_idx;
*/
  if (noDelta)
  {
    if (getToggleState())
    {
      lBuffer->setColorVal(min_start, DoubleBuffer::G/*LedBuffer::GREEN*/, COLOR_RES);
    }
    else
    {
      lBuffer->setColorVal(hour_start, DoubleBuffer::R/*LedBuffer::RED*/, COLOR_RES);
    }
  }
  else
  {
    ...
    {
      lBuffer->setColorVal(min_idx, DoubleBuffer::G/*LedBuffer::GREEN*/, min_val);
    ...
    {
      lBuffer->setColorVal(hour_idx, DoubleBuffer::R/*LedBuffer::RED*/, hour_val);
