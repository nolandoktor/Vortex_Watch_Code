#ifndef ADAFRUIT_FREETOUCH_ALT_H
#define ADAFRUIT_FREETOUCH_ALT_H

#include "adafruit_ptc.h"
#include <Arduino.h>

class Adafruit_FreeTouch_alt {
public:
  Adafruit_FreeTouch_alt(int p = 0, oversample_t f = OVERSAMPLE_4,
                     series_resistor_t r = RESISTOR_0,
                     freq_mode_t fh = FREQ_MODE_NONE);
  Adafruit_FreeTouch_alt(EPortType ulPort, uint32_t ulPin, oversample_t f = OVERSAMPLE_4,
                     series_resistor_t r = RESISTOR_0,
                     freq_mode_t fh = FREQ_MODE_NONE);
  bool begin(void);
...
private:
  ...
  int getYLine(void);
  int getYLine(EPortType ulPort, uint32_t ulPin);
